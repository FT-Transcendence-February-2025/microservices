...
export $(egrep '(SSL|DATA|ADMIN_EMAIL|AUTH_ENV|TRAEFIK_DASHBOARD)=' ./secrets/.env.tmp)
export DOMAIN=$(hostname)
IP=$(ip route get 8.8.8.8 | awk '{print $7}')
echo "# --- COMPOSE & TRAEFIK SERVICE --- #" >> .env
grep -vE '^(TOKEN|AUTH_ENV|TRAEFIK_DASHBOARD)=' ./secrets/.env.tmp >> .env
...
DOMAIN=$DOMAIN
IP=$IP
TRAEFIK_DASHBOARD=$(echo "$TRAEFIK_DASHBOARD" | sed 's/\$/\$\$/g')
# ---------- VOLUMES ---------- #
