...
const isLocalGame = urlParams.get('local') === 'true'
...
if (isLocalGame) {
    console.log("Starting local game mode for player: ", playerId)
    // Optionally hide element not needed in local mode
    const localGame = new LocalPongGame(canvas, ctx)
    localGame.start()
} else if (matchId && playerId) {

    const socket = new WebSocket(`ws://${window.location.hostname}:3003/games/${matchId}?playerId=${playerId}`)
    ...
    console.error("Missing matchId or playerId in the URL")
}
